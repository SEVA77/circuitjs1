...
//		    modelNameTextBox.setText(model.name);
    ...
    void createPinsFromModel() {
        ...
        for (int i = 0; i != postCount; i++) {
            ...
            chip.volts[i] = 0;
            if (i == selectedPin) chip.pins[i].selected = true;
        }
        chip.setPoints();
    }
    ...
    void adjustChipSize(int dx, int dy) {
        ...
        if (chip.sizeX + dx < 1 || chip.sizeY + dy < 1) return;
        model.sizeX += dx;
        model.sizeY += dy;
        createPinsFromModel();
        drawChip();
    ...
    void mouseMoved(int x, int y) {
        if (dragging) {
            ...
            int pos[] = new int[2];
            if (!chip.getPinPos((int) (x * scale), (int) (y * scale), selectedPin, pos)) return;
            ...
            if (selectedPin >= 0) chip.pins[selectedPin].selected = true;
            drawChip();
        }
    }
