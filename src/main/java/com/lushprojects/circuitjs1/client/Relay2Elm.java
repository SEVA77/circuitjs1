...
public class Relay2Elm extends CircuitElm {
    ...
    int nCoil4, nCoil5, nCoil6;
    double currentOffset1, currentOffset2;

    public Relay2Elm(int xx, int yy) {
        super(xx, yy);
        ind = new Inductor(sim);
        ind2 = new Inductor(sim);
        inductance = .2;
        ind.setup(inductance, 0, Inductor.FLAG_BACK_EULER);
        ind2.setup(inductance, 0, Inductor.FLAG_BACK_EULER);
        noDiagonal = true;
        onCurrent = .02;
        offCurrent = .015;
        r_on = .05;
    ...
    public Relay2Elm(int xa, int ya, int xb, int yb, int f,
        ...
        ind2 = new Inductor(sim);
        ind2.setup(inductance, coilCurrent, Inductor.FLAG_BACK_EULER);
        setupPoles();
        allocNodes();
    }
...
//		interpPoint(coilLeads[i], coilPosts[i], point1, .5,
//				-10);
        }
        int x = ((flags & FLAG_SWAP_COIL) != 0) ? 1 : 0;
        int x1 = ((flags & FLAG_SWAP_COIL) != 0) ? 3 : 2;
        ...
        drawCoil(g, dsign * 6, coilLeads[x1], coilLeads[5 - x1],
                volts[nCoil1 + x1], volts[nCoil2 - x1]);
        // draw rectangle
...
//	    drawDots(g, coilLeads[2], coilPosts[2], coilCurCount);
//	    drawDots(g, coilLeads[2], coilPosts[3], addCurCount(coilCurCount, currentOffset1));
//	    drawDots(g, coilLeads[3], coilPosts[3], addCurCount(coilCurCount, currentOffset1));
        }

        drawPosts(g);
        setBbox(outline[0], outline[2], 0);
        adjustBbox(coilPosts[0], coilPosts[1]);
        adjustBbox(coilPosts[2], coilPosts[3]);
    ...
    void setPoints() {
        ...
        // coil
        coilPosts = newPointArray(4);
        coilLeads = newPointArray(4);
...
//	    interpPoint(point1, point2, coilLeads[1], .5, openhs*3);
            interpPoint(point1, point2, coilLeads[2], .5, openhs * 4);
            interpPoint(point1, point2, coilLeads[3], .5, openhs * 5);
            boxSize = 56;
        } else {
            ...
            interpPoint(point1, point2, coilPosts[2], 0, openhs * 3);
            interpPoint(point1, point2, coilPosts[3], 1, openhs * 3);
            interpPoint(point1, point2, coilLeads[2], .5 - 16 / dn, openhs * 3);
            interpPoint(point1, point2, coilLeads[3], .5 + 16 / dn, openhs * 3);
            boxSize = 60;
        }
    ...
    void reset() {
        ...
        ind.reset();
        ind2.reset();
        coilCurrent = coilCurrent2 = coilCurCount = 0;
        int i;
        for (i = 0; i != poleCount; i++)
...
//	sim.stampResistor(nodes[nCoil3], nodes[nCoil2], coilR);
        ...
        // inductor from coil post 1 to internal node
        ind.stamp(nodes[nCoil1], nodes[nCoil6]);
        sim.stampResistor(nodes[nCoil6], nodes[nCoil2], coilR);

        ind2.stamp(nodes[nCoil3], nodes[nCoil5]);
        sim.stampResistor(nodes[nCoil5], nodes[nCoil4], coilR);
        // resistor from internal node to coil post 2
//        sim.stampResistor(nodes[nCoil3], nodes[nCoil2], coilR);
    ...
    void startIteration() {
        ...
        ind.startIteration(volts[nCoil1] - volts[nCoil6]);
        ind2.startIteration(volts[nCoil3] - volts[nCoil5]);
        double absCurrent = Math.abs(coilCurrent);
        double absCurrent2 = Math.abs(coilCurrent2);

        if (onState) {
            // on or turning on.  check if we need to turn off
            if (absCurrent < offCurrent && absCurrent2 < offCurrent) {
...
//                    d_position = i_position = 1;
                d_position = f_position = i_position = 1;
            }
        } else {
            // off or turning off.  check if we need to turn on
            if (absCurrent > onCurrent || absCurrent2 > onCurrent) {
...
//	ind.doStep(voltdiff);
        ...
        double voltdiff = volts[nCoil1] - volts[nCoil6];
        double voltdiff2 = volts[nCoil3] - volts[nCoil5];

        ind.doStep(voltdiff);
        ind2.doStep(voltdiff2);
        int p;
    ...
    void calculateCurrent() {
        ...
        coilCurrent2 = ind2.calculateCurrent(voltdiff2);
    ...
    boolean comparePairNew(int a1, int a2, int b1, int b2) {
        return ((a1 == b1 && a2 == b2) || (a1 == b2 && a2 == b1));
    }

    boolean getConnection(int n1, int n2) {
        if (comparePairNew(n1, n2, 3 * poleCount, 3 * poleCount + 1))
            return true;
        if (comparePairNew(n1, n2, 3 * poleCount + 2, 3 * poleCount + 3))
            return true;
        if (n1 >= 3 * poleCount || n2 >= 3 * poleCount) {
            return false;
        }
        return (n1 / 3 == n2 / 3);
    }
